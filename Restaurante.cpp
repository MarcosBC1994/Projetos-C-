...
void Restaurante::inserirPedidoCliente() {
    ...
    while (!(cin >> numMesa)) {
        ...
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');//descartar carateres existentes
    }
    ...
    while (true) {
        ...
        }// verifica se o nome do cliente contem apenas letras e espacos
    ...
    while (true) {
        ...
        // Verifica se o telefone do cliente contém apenas números e se tem 9 dígitos
    ...
    // getline premite ler uma linha inteira, incluindo espacos
    ...
    do {
        ...
        while (!(cin >> opcaoPrato) || opcaoPrato < 1 || opcaoPrato > ementaPratos.size() || ementaPratos[opcaoPrato - 1].quantidadeEmStock <= 0) {
            ...
            }// verifica se a opcao escolhida esta dentro da ementa
            ...
            }// verifica se o prato escolhido esta disponivel
            else {
    ...
    if (opcaoPrato >= 1 && opcaoPrato <= ementaPratos.size()) {
        // Verificar se o prato está disponível em stock
        ...
        }// Verifica se o pedido foi sorteado para não pagamento
    }
    else {
        cout << "\nOpcao invalida!\n";
...
void Restaurante::imprimirRecibo() const {
    ...
    // Armazena o resultado da mesa
    bool mesaEncontrada = false;
    // Armazena o resultado da sorteio
    ...
    for (const auto& pedido : pedidos) {
        if (pedido.getNumMesa() == opcaoMesa) {
            if (!mesaEncontrada) {
                ...
                cout << fixed << setprecision(2);
                mesaEncontrada = true;
                pedidoSorteado = verificarsorteado(pedido); // Armazena o resultado
            }
...
void Restaurante::adicionarStockaPrato() {
    if (ementaPratos.empty()) {
        cout << "\nA ementa está vazia. Não há pratos para ver ou adicionar stock.\n";
        return;
    }

    cout << "\nEscolha o número do prato que deseja ver ou adicionar stock:\n";
    for (size_t i = 0; i < ementaPratos.size(); ++i) {
...
void Restaurante::addCliente()
{
    ...
    while (true) {
        ...
        if (all_of(nomeCliente.begin(), nomeCliente.end(), [](char c) { return isalpha(c) || isspace(c); })) {
            break; // Sai do loop se o input contiver apenas letras e espaços
        }
        else {
            cout << "Por favor, digite um nome válido contendo apenas letras e espaços: ";
        }
    }
    ...
    while (true) {
        ...
        if (all_of(telefoneCliente.begin(), telefoneCliente.end(), [](char c) { return isdigit(c); }) && telefoneCliente.size() == 9) {
            ...
            cout << "Por favor, digite um numero de telefone valido com exatamente 9 digitos: ";
        }
    }
...
void Restaurante::imprimirClientes() {
    ...
    for (const auto& cliente : clientes) {
        cout << "Nome: " << cliente.getNome() << ", Telefone: " << cliente.getTelefone() << ", Endereco: " << cliente.getEndereco() << ", Numero: "<< cliente.getNumCliente();
    }
}
...
// Ler o ID do funcionário
    ...
    if (idFuncionario != 1234) {
        cout << setw(75) << "ID incorreto. Saindo...\n";
        system("pause");
        system("cls");
    }
    cout << "\033[0m";
    ...
    cout << setw(60) << "Verificando senha";
    for (int i = 0; i < 3; ++i) {
        cout << ".";
    ...
    if (senhaFuncionario != 1234) {
        cout << setw(75) << "Senha incorreta. Saindo...\n";
        system("pause");
        system("cls");
    }
    cout << "\033[0m";
    ...
    cout << setw(60) << "Preparando o sistema...";
    for (int i = 0; i < 3; ++i) {
        cout << ".";
        this_thread::sleep_for(chrono::seconds(1));
    }
